...
import graph.model.pixelcnn as pixelcnn
...
class BaseTrainer:
    ...
    def __init__(self, resume, config, train_logger=None):
        ...
        self.gen = module_arch.Generator_mnist(self.config)
        self.pixelcnn = pixelcnn.PixelCNN(nr_resnet=3, disable_third=True, dropout_p=0.0, n_channel=1, image_wh=28)
        ...
        self.pixelcnn = model_parallel(self.pixelcnn, self.device, self.device_ids)

        #load pixelcnn dict
        self.pixelcnn.load_state_dict(torch.load(self.config['model']['pixelcnn_path']))

        with torch.no_grad():
            log_probs_list = []
            for dev_images, _ in self.dev_loader.get_iter():
                dev_images = dev_images.to(self.device)
                dev_images = (dev_images - 0.5) / 0.5
                dev_images = dev_images.view(-1, 1, 28, 28)
                logits = self.pixelcnn(dev_images)
                log_probs = - module_loss.discretized_mix_logistic_loss_c1(dev_images.permute(0, 2, 3, 1),
        ...
        # Save configuration file into writer directory:
        config_save_path = os.path.join(writer_dir, 'config.json')
