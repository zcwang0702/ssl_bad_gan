...
class PixelCNN(nn.Module):
    ...
    def down_shifted_conv2d(self, x, num_filters, filter_size=(2, 3), stride=(1, 1), **kwargs):
        ...
        if module is None:
            module = WN_Conv2d(x.size(1), num_filters, filter_size, stride, train_scale=True, **kwargs).cuda()
    ...
    def down_shifted_deconv2d(self, x, num_filters, filter_size=(2, 3), stride=(1, 1), **kwargs):
        module = getattr(self, str(self.counter), None)
        if module is None:
            module = WN_ConvTranspose2d(x.size(1), num_filters, filter_size, stride, output_padding=1,
    ...
    def nin(self, x, num_units, **kwargs):
        module = getattr(self, str(self.counter), None)
        if module is None:
            module = WN_Conv2d(x.size(1), num_units, 1, train_scale=True, **kwargs).cuda()
            self.add_module(str(self.counter), module)
        self.counter += 1
        xs = x.size()
        return module(x)
