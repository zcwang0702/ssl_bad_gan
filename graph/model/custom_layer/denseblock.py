...
class DenseBlock(nn.Sequential):
    def __init__(self, num_layers, in_channels, growth_rate, config):
        ...
        for layer_id in range(num_layers):
            layer = DenseLayer(in_channels=in_channels + (layer_id * growth_rate), growth_rate=growth_rate,
                               config=config)
...
class DenseLayer(nn.Module):
    def __init__(self, in_channels, growth_rate, config):
        ...
        self.conv_1 = LearnedGroupConv(in_channels=in_channels, out_channels=self.conv_bottleneck * growth_rate,
                                       kernel_size=1,
        ...
        self.conv_2 = nn.Conv2d(in_channels=self.conv_bottleneck * growth_rate, out_channels=growth_rate, kernel_size=3,
                                padding=1, stride=1, groups=self.group3x3, bias=False)
